import { mysqlTable, text, int, boolean, timestamp, mysqlEnum, json, unique, varchar } from "drizzle-orm/mysql-core";
...
export const emojiTypeEnum = mysqlEnum('emoji_type', [
  'thumbs_up', 'thumbs_down', 'heart', 'tada', 'thinking', 'smile', 'cry', 'clap'
]);
...
export const users = mysqlTable("users", {
  id: int("id").primaryKey().autoincrement(),
  username: varchar("username", { length: 255 }).notNull().unique(), // <--- corrigido
  password: text("password").notNull(),
  email: varchar("email", { length: 255 }).notNull().unique(),       // <--- corrigido
  fullName: text("full_name").notNull(),
  role: userRoleEnum.notNull().default('entity_member'),
  phone: text("phone"),
...
export const achievementBadges = mysqlTable("achievement_badges", {
  id: int("id").primaryKey().autoincrement(),
  name: varchar("name", { length: 255 }).notNull().unique(), // <-- corrigido aqui!
  description: text("description").notNull(),
...
export const publicHearings = mysqlTable("public_hearings", {
  ...
  status: publicHearingStatusEnum.default('scheduled').notNull(),
...
export const storage = new DatabaseStorage(); // <-- Isso usa o banco real!
