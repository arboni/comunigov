...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const users = await db
        .select([
          ...
          usersTable.phone,
          usersTable.entityId,
          entitiesTable.id.as("entity_id"),
          entitiesTable.name.as("entity_name"),
        ])
        .from(usersTable)
  ...
  app.get("/api/dashboard/stats", isAuthenticated, async (req, res, next) => {
    try {
      // Use Drizzle para contar registros diretamente no banco
      ...
      const [{ count: upcomingMeetings }] = await db.select({ count: sql<number>`COUNT(*)` }).from(meetingsTable);
      const [{ count: pendingTasks }] = await db.select({ count: sql<number>`COUNT(*)` }).from(tasksTable);
