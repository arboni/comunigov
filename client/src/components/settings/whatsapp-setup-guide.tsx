import React from "react";
import {
  Alert,
  AlertDescription,
  AlertTitle,
} from "@/components/ui/alert";
import { InfoIcon, ExternalLinkIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";

interface WhatsAppSetupGuideProps {
  twilioNumber?: string;
  joinCode?: string;
}

export default function WhatsAppSetupGuide({ 
  twilioNumber = "Twilio WhatsApp number", 
  joinCode = "forgotten-clock" 
}: WhatsAppSetupGuideProps) {
  return (
    <Alert className="bg-white border-blue-200 mb-6">
      <InfoIcon className="h-4 w-4 text-blue-500" />
      <AlertTitle className="text-blue-700 font-semibold text-sm flex items-center">
        WhatsApp Setup Required
      </AlertTitle>
      <AlertDescription className="text-sm mt-2">
        <p className="mb-2">
          Before you can receive WhatsApp messages from ComuniGov, you need to join the Twilio WhatsApp Sandbox by following these steps:
        </p>
        
        <ol className="list-decimal pl-5 mb-3 space-y-2">
          <li>Open WhatsApp on your phone</li>
          <li>
            Send the following message to <span className="font-semibold">{twilioNumber}</span>:
            <div className="bg-gray-100 p-2 my-2 rounded font-mono text-sm">
              join {joinCode}
            </div>
          </li>
          <li>You should receive a confirmation message from Twilio</li>
          <li>Once confirmed, you can receive WhatsApp messages from ComuniGov</li>
        </ol>
        
        <Separator className="my-3" />
        
        <div className="text-xs text-gray-600">
          <p className="mb-1">
            Note: This is a Twilio Sandbox requirement. Your WhatsApp number will only receive messages after joining.
          </p>
          <p>
            If you don't complete this setup, your communications will still be sent via email.
          </p>
        </div>
        
        <div className="mt-3">
          <Button 
            variant="outline" 
            size="sm"
            className="text-xs h-7"
            onClick={() => window.open("https://www.twilio.com/docs/whatsapp/sandbox", "_blank")}
          >
            Learn More <ExternalLinkIcon className="ml-1 h-3 w-3" />
          </Button>
        </div>
      </AlertDescription>
    </Alert>
  );
}