...
/**
 * Clean and normalize CSV column headers
 * Handles various formats and special characters that might be present in CSV files
 * @param header The column header to clean
 * @returns Cleaned header string
 */
function cleanHeaderField(header: string): string {
  // Remove quotes around the column name
  const unquoted = header.replace(/^"(.*)"$/, '$1').trim();
  
  // Remove any BOM (Byte Order Mark) characters that might be present
  const noBom = unquoted.replace(/^\uFEFF/, '');
  
  // Remove any non-printable characters
  const noControlChars = noBom.replace(/[\x00-\x1F\x7F]/g, '');
